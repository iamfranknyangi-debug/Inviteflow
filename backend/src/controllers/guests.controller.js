...
// POST /api/guests/bulk — CSV only (no xlsx dependency)
exports.bulkUpload = async (req, res, next) => {
  try {
    ...
    try {
      ...
      return res.status(400).json({ success:false, message:'Could not parse file. Please use CSV format.' });
    }
    ...
    processBulkRecords(records, event_id, jobId).catch(e => logger.error('Bulk error', e.message));
...
exports.generateQR = async (req, res, next) => {
  try {
    ...
    try { qr_image_url = await uploadBuffer(qrBuffer, `qr/${guest.id}.png`); } catch (e) {}
